...
public class DateUtil {
        ...
        // ƒата начала текущего учебного года
    ...
    public static Date getCurrEducDate(Date p_rep_date, int p_educ_year_begin_month, int p_educ_year_begin_day) {
        ...
        // ƒата начала учебного года в текущем году
...
//        clearCalendarTimePart(x_curr_year_educ_begin);
        ...
        // ќпределить текущий учебный год
    ...
    public static Date getCurrentDate(Boolean zero) {
        Date date = new Date();                      // текущее врем€
        if (zero) {
            Calendar cal = Calendar.getInstance();       // экземпл€р календар€
            cal.setTime(date);                           // набор временных параметров
            clearCalendarTimePart(cal);                  // обнул€ем часы, минуты, секунды, милисекунды
            return cal.getTime();
        } else {
            return date;
        }
    ...
    /**
     * »зменить дату на указанное кол-во лет, мес€цев и дней
     * @param p_date дата
     * @param p_year лет
     * @param p_month мес€цев
     * @param p_days дней
     * @return 
     */    
